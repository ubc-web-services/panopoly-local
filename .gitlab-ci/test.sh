...
if [[ "$UPGRADE" = "no-upgrade" ]]; then
	...
	# If an old version of Panopoly failed to install panopoly_demo (for example,
	# if the apps server URL was wrong) we need to make sure it gets installed.
	lando drush en -y panopoly_demo
fi
...
lando drush vset -y error_level "0"
