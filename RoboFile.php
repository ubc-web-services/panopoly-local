...
class RoboFile extends RoboTasks {
  ...
  const GPL_V2_LICENSE_URL = 'https://www.gnu.org/licenses/old-licenses/gpl-2.0.txt';
  ...
  public function checkMakefile() {
    ...
    if ($process->getExitCode() !== 0) {
      throw new \Exception("'drush verify-makefile drupal-org.make' fails!");
    }
  }
  ...
  public function checkOverridden() {
    ...
    foreach ($this->getPanopolyFeatures() as $panopoly_feature) {
      ...
      if ($process->getExitCode() != 0 || strpos($output, "Feature is in its default state") === FALSE) {
...
EOF;
   ...
   *
   * @option string $source-path
   *   The path to the profile directory to build from. Defaults to the current
   *   directory.
   * @option bool $no-build-drupal-org-make
   *   If used, we won't build the drupal-org.make file before building the
   *   dependencies.
   */
  public function buildDependencies($opts = ['source-path' => NULL, 'no-build-drupal-org-make' => FALSE]) {
    /** @var $this|\Robo\Collection\CollectionBuilder $collection */
    $collection = $this->collectionBuilder();

    if (!empty($opts['source-path']) && empty($opts['no-build-drupal-org-make'])) {
      $opts['no-build-drupal-org-make'] = TRUE;
      $this->io()->warning("build:dependencies with --source-path implies --no-build-drupal-org-make");
    }
    $source_path = $opts['source-path'] ?? getcwd();

    if (!$opts['no-build-drupal-org-make']) {
   ...
   * @throws \Exception
   */
  public function buildSite($target_path, $opts = ['source-path' => NULL]) {
    ...
    $source_path = $opts['source-path'] ?? __DIR__;
    $destination_path = "{$target_path}/profiles/panopoly";
    $collection->taskExec("drush make {$source_path}/drupal-org-core.make {$target_path} -y");
    $collection->taskMirrorDir([$source_path => $destination_path]);

    $build_dependencies_args = [];
    if (!empty($opts['source-path'])) {
      $build_dependencies_args['source-path'] = $destination_path;
      $build_dependencies_args['no-build-drupal-org-make'] = TRUE;
    }
    $collection->addTask($this->buildDependencies($build_dependencies_args));
   ...
   *   The path to place the zips and tarballs.
   *
   * @option string $datestamp
   *   If passed, the datestamp to use. Otherwise, the current time is used.
   * @option string $project
   *   The project.
   * @option string $core
   *   The core version.
   */
  public function package($version, $output_path, $opts = ['datestamp' => NULL, 'project' => 'panopoly', 'core' => '7.x']) {
    $collection = $this->collectionBuilder();

    // Strip trailing slashes.
    $output_path = rtrim($output_path, "/");

    // Create the profile directory, and add packaging info and license.
    $temp_path = $collection->tmpDir();
    $profile_path = "{$temp_path}/panopoly";
    $collection->addTask($this->packageMirrorProfile($profile_path));
    $collection->addTask($this->packageUpdateInfo($version, $profile_path, $opts));
    $collection->addTask($this->packageDownloadLicense($profile_path));

    // Build the full source with core and all the dependencies.
    $site_path = "{$temp_path}/panopoly-{$version}";
    $collection->addTask($this->buildSite($site_path, ['source-path' => $profile_path]));

    // Create the zip's and tarballs.
    $collection->taskFileSystemStack()->mkdir($output_path);
    $collection->taskPack("{$output_path}/panopoly-{$version}.zip")
      ->addDir("panopoly", $profile_path);
    $collection->taskPack("{$output_path}/panopoly-{$version}.tar.gz")
      ->addDir("panopoly", $profile_path);
    $collection->taskPack("{$output_path}/panopoly-{$version}-core.zip")
      ->addDir("panopoly-{$version}", $site_path);
    $collection->taskPack("{$output_path}/panopoly-{$version}-core.tar.gz")
      ->addDir("panopoly-{$version}", $site_path);
    $collection->taskPack("{$output_path}/panopoly-{$version}-no-core.zip")
      ->addDir("panopoly", "{$site_path}/profiles/panopoly");
    $collection->taskPack("{$output_path}/panopoly-{$version}-no-core.tar.gz")
      ->addDir("panopoly", "{$site_path}/profiles/panopoly");
  ...
  /**
   * Mirror the clean profile to another directory.
   *
   * @param string $path
   *   The path to write the files.
   */
  public function packageMirrorProfile($path) {
    $collection = $this->collectionBuilder();
    ...
    $collection->taskFileSystemStack()->mkdir($path);
    $collection->taskExec("git checkout-index -a -f --prefix={$path}");
    return $collection;
  }

  /**
   * Updates .info files with packaging information.
   *
   * @param string $version
   *   The package version.
   * @param string $path
   ...
   *   The core version.
   */
  public function packageUpdateInfo($version, $path, $opts = ['datestamp' => NULL, 'project' => 'panopoly', 'core' => '7.x']) {
    $datestamp = $opts['datestamp'] ?? time();
    $date = date('Y-m-d', $datestamp);
    $content  = "\n; Information added by Panopoly packaging script on {$date}\n";
    $content .= "version = \"{$version}\"\n";
    $content .= "core = \"{$opts['core']}\n";
    $content .= "project = \"{$opts['project']}\"\n";
    $content .= "datestamp = \"{$datestamp}\"\n";

    $collection = $this->collectionBuilder();
    $collection->addCode(function () use ($path, $content) {
      $realpath = realpath($path);
      $directory = new RecursiveDirectoryIterator($realpath);
      $iterator = new RecursiveIteratorIterator($directory);
      $filtered = new CallbackFilterIterator($iterator, function ($current, $key, $iterator) use ($realpath) {
        if ($current->isDir()) {
          return FALSE;
        }
        if ($current->getExtension() !== 'info') {
          return FALSE;
        }
        if (preg_match('|^' . preg_quote($realpath, '|') . '/modules/contrib/|', $current->getRealPath())) {
          return FALSE;
        }
      ...
      foreach ($filtered as $info) {
        file_put_contents($info->getRealPath(), $content, FILE_APPEND | LOCK_EX);
      }
    });
  ...
  /**
   * Downloads the GPLv2 LICENSE.txt into the given path.
   *
   * @param string $path
   *   The directory to create the LICENSE.txt in.
   */
  public function packageDownloadLicense($path) {
    $collection = $this->collectionBuilder();
    $collection->taskWriteToFile("{$path}/LICENSE.txt")
      ->text(file_get_contents(static::GPL_V2_LICENSE_URL));
    return $collection;
  }
